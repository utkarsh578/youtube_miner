...
1. http://35.226.20.165:3000/api/v1/youtube/data/fetch?per_page_count=[value]&page_no=[value]
2. Click here(http://35.226.20.165:3000/api/v1/youtube/data/fetch?per_page_count=20&page_no=1) for example 
3. Also i had also exposed one more API to check the count in the database this verifies that the job server is running in      background and updating the data(after every 5 minutes) in db
